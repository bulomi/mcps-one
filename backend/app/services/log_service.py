...
class LogService:
    ...
    def create_mcp_log(self, log_data: MCPLogCreate) -> MCPLog:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(f"创建 MCP 协议日志失败: {e}")
    ...
    def export_logs(
        ...
        try:
            # 获取日志数据
            if log_type == "system":
                logs, _ = self.get_system_logs(page=1, size=10000, filters=filters)
                ...
                logs, _ = self.get_operation_logs(page=1, size=10000, filters=filters)
                ...
                logs, _ = self.get_mcp_logs(page=1, size=10000, filters=filters)
    ...
    def _apply_mcp_log_filters(self, query, filters: Dict[str, Any]):
        ...
        if filters.get('has_error'):
            if filters['has_error']:
                query = query.filter(MCPLog.error_code.isnot(None))
            else:
                query = query.filter(MCPLog.error_code.is_(None))
