...
class TaskService:
    ...
    def get_recent_tasks(self, limit: int = 10) -> List[MCPTask]:
        """获取最近的任务"""
        return self.db.query(MCPTask).order_by(
            desc(MCPTask.created_at)
        ).limit(limit).all()
    
    def get_tasks_by_session(self, session_id: int, limit: int = 50) -> List[MCPTask]:
        """获取会话相关的任务"""
        return self.db.query(MCPTask).filter(
            MCPTask.session_id == session_id
        ).order_by(desc(MCPTask.created_at)).limit(limit).all()
    
    def get_tasks_by_tool(self, tool_id: int, limit: int = 50) -> List[MCPTask]:
        """获取工具相关的任务"""
        return self.db.query(MCPTask).filter(
